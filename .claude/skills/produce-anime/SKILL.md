---
name: produce-anime
description: 短剧制作技能。用于生成完整短剧作品，包括剧本编写、角色设计、6宫格分镜、故事板配置。每次运行生成1部作品（25集，每集30秒=上下两部分各15秒，每部分6宫格分镜）。关键词：短剧、影视、drama、剧本、分镜、storyboard、角色设计。
---
...
- 每部分包含 **6宫格分镜提示词**
- 每集生成 **3个文件**：对话脚本 + 故事板配置 + Seedance任务JSON（每集2条：Part-A/B）
- 含氛围描述、中文人物对话、无字幕
- 视频编号管理索引
...
└── DM-001_xxxx/                        # 单部作品目录
    ...
    │   │   └── storyboard_config.json  # 故事板配置（含上下两部分，每部分6宫格）
    │   │   └── seedance_tasks.json     # Seedance提交任务（每集2条：Part-A/B）
    │   ├── EP02/
    │   │   └── ...
    │   └── ... (EP01-EP25)
...
```json
{
  ...
  "part_a": {
    ...
    "video_prompt": "English prompt for AI video generation of Part A (15s). No subtitles.",
    "bgm": {
      "description": "背景音乐描述",
      "mood": "音乐情绪关键词"
    },
  ...
  "part_b": {
    ...
    "video_prompt": "English prompt for AI video generation of Part B (15s). No subtitles.",
    "bgm": {
      "description": "背景音乐描述",
      "mood": "音乐情绪关键词"
    },
...
- [ ] 每部分包含6宫格分镜
- [ ] 每集 `seedance_tasks.json` 含 2 条任务（Part-A/B）
